    	...
    	<div class="top">
            ...
            <div class="top_menu">
            	<ul>
                    ...
                    <li><a href="mail!adminmaillist" style="background-color:#226411;">邮件管理</a></li>
        ...
        <div class="content" style="overflow-y:auto; overflow-x:auto; height:480px;"><%--style="overflow:scroll;" --%>
        	<div class="user_info_title">
                ...
                <table border="1" cellspacing="0" bordercolor="#D1DDAA" width = "80%" style="border-collapse:collapse;"  >
                	<tr bgcolor="#ffffff"><td>选择</td><td>用户名</td><td>邮件编号</td><td>标题</td><td>邮件类型</td><td>发送时间</td><td>发送给</td><td>邮件状态</td><td>修改</td><td>删除</td></tr>
                    <c:forEach items="${requestScope.list }" var="vo">
                    <tr style="font-size: 13px;"><td><input type="checkbox"/></td><td>${vo.uName }</td><td>${vo.eId }</td><td>${vo.eTitle}</td><td>${vo.eStyle }</td><td>${vo.eTotime }</td><td>${vo.eTouname }</td><td>${vo.eState }</td><td><a href="usersinfoupdate?uname=${vo.uName }">修改</a></td><td><a href="usersinfodelete?uname=${vo.uName}" onclick="return window.confirm('您确定要删除吗？')">删除</a></td></tr>
                    </c:forEach>
