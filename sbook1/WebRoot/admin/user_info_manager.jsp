...
<html>
  ...
  <body>
    <div class="all">
    	<div class="top">
        	<div class="top_link"><span>您好[${sessionScope.admin }]，欢迎使用同学录管理系统！&nbsp;<a href="login.jsp">[网站前台]</a><a href="login.jsp">[退出登录]</a><a href="login.jsp">[注销登录]</a></span></div>
            <div class="top_menu">
            	<ul>
        ...
        <div class="content" style="overflow-y:auto; overflow-x:auto; height:480px;"><%--style="overflow:scroll;" --%>
        	<div class="user_info_title">
                ...
                <table border="1" cellspacing="0" bordercolor="#D1DDAA" width = "80%" style="border-collapse:collapse;"  >
                	<tr bgcolor="#ffffff"><td>选择</td><td>用户名</td><td>用户邮件</td><td>年龄</td><td>性别</td><td>就读院校</td><td>曾读院校</td><td>生日</td><td>QQ号</td><td>手机号码</td><td>修改</td><td>删除</td></tr>
                    <c:forEach items="${requestScope.list }" var="vo">
                    <tr style="font-size: 13px;"><td><input type="checkbox"/></td><td>${vo.uName }</td><td>${vo.uEmail }</td><td>${vo.uAge}</td><td>${vo.uSex }</td><td>${vo.uReadingschool }</td><td>${vo.uReadschool }</td><td>${vo.uBirth }</td><td>${vo.uQq }</td><td>${vo.uTelphone }</td><td><a href="user!usersinfoupdate?uname=${vo.uName }">修改</a></td><td><a href="user!usersinfodelete?uname=${vo.uName}" onclick="return window.confirm('您确定要删除吗？')">删除</a></td></tr>
                    </c:forEach>
